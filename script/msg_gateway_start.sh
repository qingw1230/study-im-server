...
list1=$(cat $config_path | grep onlineMessageRelayPort | awk -F '[:]' '{print $NF}')
list2=$(cat $config_path | grep wsPort | awk -F '[:]' '{print $NF}')
...
for ((i = 0; i < ${#ws_ports[@]}; i++)); do
  ./${msg_gateway_name} -rpc_port ${rpc_ports[$i]} -ws_port ${ws_ports[$i]} &
done
